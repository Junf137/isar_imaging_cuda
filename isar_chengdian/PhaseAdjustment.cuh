#ifndef PHASEADJUSTMENT_H_
#define PHASEADJUSTMENT_H_
...
struct BuildRangeVector :public thrust::unary_function<float, float>
{
	...
	{
		return x * 2.0 * resolution / wave_length;
	}
};
...
struct GetAngle :public thrust::binary_function<float, float, float>
{
	...
	{
		float x = sinf(cur_pit / 180 * PI);
		float y = cosf(cur_pit / 180 * PI) * cosf(cur_azi / 180 * PI);
		float z = cosf(cur_pit / 180 * PI) * sinf(cur_azi / 180 * PI);
...
struct CompensatePhase :public thrust::binary_function<float, comThr, comThr>
{
	...
	{
		return y * thrust::exp(comThr(0.0, -2 * PI * x));
	}
};
