...
void Doppler_Tracking(cuComplex* d_data, RadarParameters paras)
{
	...
	//checkCublasErrors(cublasCgeam(handle, CUBLAS_OP_T, CUBLAS_OP_T, paras.num_range_bins, paras.num_echoes, &alpha_trans_data,
	...
	conjComplex_Mul_Complex op_cm;
	thrust::transform(thrust::device, thr_d_Data,
		thr_d_Data + (paras.num_echoes - 1) * paras.num_range_bins, thr_d_Data + paras.num_range_bins, mulRes.begin(), op_cm);
	...
	//checkCublasErrors(cublasCgeam(handle, CUBLAS_OP_T, CUBLAS_OP_T, paras.num_echoes, paras.num_range_bins, &alpha_trans_data,
...
void RangeVariantPhaseComp(cuComplex* d_data, RadarParameters paras, float* azimuth_data, float* pitch_data)
{
	...
	float mid_x = sinf(middle_pitch / 180 * PI);
	float mid_y = cosf(middle_pitch / 180 * PI) * cosf(middle_azimuth / 180 * PI);
	float mid_z = cosf(middle_pitch / 180 * PI) * sinf(middle_azimuth / 180 * PI);
	...
	checkCudaErrors(cudaMemset(comp_mat, 0.0, sizeof(float) * paras.num_echoes * paras.num_range_bins));
...
void Fast_Entropy(cuComplex* d_data, RadarParameters paras)
{
	...
	// abs(RetData_RA)
	Get_abs op_abs;
	thrust::transform(thrust::device, thr_Data, thr_Data + NumEcho * NumRange, thr_DataAbs.begin(), op_abs);
	...
	int tgt_num = end - tgt_index.begin();
	...
	checkCuFFTErrors(cufftPlanMany(&plan, rank_img, n_img,
		inembed_img, istride_img, idist_img,
		onembed_img, ostride_img, odist_img,
		CUFFT_C2C, batch_img));
	checkCuFFTErrors(cufftExecC2C(plan, d_preComData, Image1, CUFFT_FORWARD));
	...
	checkCublasErrors(cublasSgemv(handle, CUBLAS_OP_N, NumRange, NumEcho, &alpha, d_sqrtImg, NumRange, d_Ones, 1,
		&beta, d_sumImg, 1));
	...
	checkCublasErrors(cublasSger(handle, NumRange, NumEcho, &alpha, d_sumImg, 1, d_Ones, 1, d_mask_entropy, NumRange));
	...
	checkCublasErrors(cublasSgemv(handle, CUBLAS_OP_N, NumRange, NumEcho, &alpha, d_mask_entropy, NumRange, d_Ones, 1,
		&beta, d_entropy, 1));
	...
	************************************/
	...
	for (int ii = 0; ii < search_num; ii++) {
		...
		checkCuFFTErrors(cufftExecC2C(plan, (cufftComplex*)d_tempData, (cufftComplex*)d_tempData, CUFFT_FORWARD));

		thrust::transform(thrust::device, thr_tempData, thr_tempData + NumEcho * num_unit2, thr_tempData, op_cma);

		checkCuFFTErrors(cufftExecC2C(plan, (cufftComplex*)d_tempData, (cufftComplex*)d_tempData, CUFFT_INVERSE));
		...
		checkCublasErrors(cublasCgemv(handle, CUBLAS_OP_T, num_unit2, NumEcho, &alpha_c, d_tempData, num_unit2, d_rowSum, 1, &beta_c, d_phi, 1));
...
void Doppler_Tracking2(cuComplex* d_preComData, cuComplex* d_Data, int NumEcho, int NumRange, thrust::device_vector<comThr>& phaseC, int isNeedCompensation)
{
	...
	conjComplex_Mul_Complex op_cm;
	thrust::transform(thrust::device, thr_d_Data, thr_d_Data + (NumEcho - 1) * NumRange, thr_d_Data + NumRange, mulRes.begin(), op_cm);
